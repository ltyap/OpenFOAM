...
#include "omegaUWallFunctionFvPatchScalarField.H"
...
scalar omegaUWallFunctionFvPatchScalarField::tolerance_ = 1e-5;

// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void omegaUWallFunctionFvPatchScalarField::setMaster()
{
    if (master_ != -1)
    {
        return;
    ...
    forAll(bf, patchi)
    {
        if (isA<omegaUWallFunctionFvPatchScalarField>(bf[patchi]))
        {
            omegaUWallFunctionFvPatchScalarField& opf = omegaPatch(patchi);
...
void omegaUWallFunctionFvPatchScalarField::createAveragingWeights()
{
    const volScalarField& omega =
        static_cast<const volScalarField&>(this->internalField());
...
omegaUWallFunctionFvPatchScalarField&
omegaUWallFunctionFvPatchScalarField::omegaPatch(const label patchi)
{
    const volScalarField& omega =
        static_cast<const volScalarField&>(this->internalField());
    ...
    const omegaUWallFunctionFvPatchScalarField& opf =
        refCast<const omegaUWallFunctionFvPatchScalarField>(bf[patchi]);

    return const_cast<omegaUWallFunctionFvPatchScalarField&>(opf);
}


void omegaUWallFunctionFvPatchScalarField::calculateTurbulenceFields
(
...
void omegaUWallFunctionFvPatchScalarField::calculate
(
    const momentumTransportModel& turbModel,
...
)
{
    ...
    const volScalarField& fOmega = db().lookupObject<volScalarField>("fOmega");
    ...
    forAll(nutw, facei)
    {
        ...
        if (blended_)
        {
            ...
            const scalar omegaVis = fOmega[celli]*6*nuw[facei]/(beta1_*sqr(y[facei]));
            const scalar omegaLog = fOmega[celli]*uStar/(Cmu5*nutw.kappa()*y[facei]);
...
omegaUWallFunctionFvPatchScalarField::omegaUWallFunctionFvPatchScalarField
(
    const fvPatch& p,
...
omegaUWallFunctionFvPatchScalarField::omegaUWallFunctionFvPatchScalarField
(
    const omegaUWallFunctionFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
omegaUWallFunctionFvPatchScalarField::omegaUWallFunctionFvPatchScalarField
(
    const omegaUWallFunctionFvPatchScalarField& owfpsf,
    const DimensionedField<scalar, volMesh>& iF
)
:
...
scalarField& omegaUWallFunctionFvPatchScalarField::G(bool init)
{
...
scalarField& omegaUWallFunctionFvPatchScalarField::omega(bool init)
{
    if (patch().index() == master_)
    {
        if (init)
...
void omegaUWallFunctionFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
...
void omegaUWallFunctionFvPatchScalarField::updateWeightedCoeffs
(
...
void omegaUWallFunctionFvPatchScalarField::manipulateMatrix
(
...
void omegaUWallFunctionFvPatchScalarField::write(Ostream& os) const
{
